...
class NodeFrame:
    ...
    def __init__(self, parent):
        ...
        globalResetButton = Button(self.nodeFrame, text="Reset", command=lambda: self.Reset(), font=("Verdana", 10),
                                    fg='black', bg='orange')
        globalResetButton.place(relx=(1 / (NodeFrame.numberOfSubframes +1/2) + 0.0015) * (NodeFrame.numberOfSubframes), rely=1/4,
                            relwidth=1-(1 / (NodeFrame.numberOfSubframes +1/2) + 0.0015) * (NodeFrame.numberOfSubframes), relheight=1/2)
        self.refresh_label()
    ...
    class Node:
        ...
        def __init__(self, parent, name = None):
            ...
            self.nodeFrame.place(relx=(1 / (NodeFrame.numberOfSubframes +1/2) + 0.0015) * NodeFrame.Node.numberOfNodes, rely=0,
            ...
            canResetButton = Button(self.nodeFrame, text="Can Reset", command=lambda: self.Reset(), font=("Verdana", 10),
                                    fg='black', bg='white')
        ...
        def makeNodeLabels(self):
            ...
            self.nodeLabels = [0] *numberOfLabels
            for i in range(numberOfLabels):
                self.nodeLabels[i] = Label(self.nodeFrame, text=self.labelarray[i], bg="grey", anchor="w")
                self.nodeLabels[i].place(relx=0.01, rely=(1 / numberOfLabels) * i+.01, relwidth=1/3, relheight=1/(numberOfLabels+1))
...
class StateButtons:
    ...
    def VentAbortInstantiation(self):
        ...
        button = Button(self.parent, text=self.args[0], command=lambda: Main.leftframe.StateReset(),fg='red', bg='grey')#, font=self.fontSize)
    ...
    def Logic(self):
        if self.stateName == "Test":
            ...
            if self.prevState.stateName == StateButtons.CurrState or (StateButtons.CurrState=="Passive" and self.prevState.stateName == "Test"):
                self.passiveState = Label(self.parent, text="Active", bg="grey", fg="Green")#, font=fontSize)
                self.passiveState.place(relx=.1, rely=0.0125, relwidth=.8, relheight=1 / 30)
                self.StateActuaction()
        ...
        if StateButtons.CurrState != "Override":
            ...
            for i in range(len(StatesFrame.States)):
                if StatesFrame.States[i][0] == StateButtons.CurrState:
                    self.reminderButtonOfCurrState = Button(Main.leftframe.stateFrame, text= StateButtons.CurrState,fg='orange', bg='black', bd = 5)
                    # Goes to logic function when button is pressed
                    self.reminderButtonOfCurrState.place(relx=0.125/2, rely=(1/(len(StatesFrame.States)+1)*(StatesFrame.States[i][1]-1))-1/18, relheight=1 / (len(StatesFrame.States)+1), relwidth= 0.85)
