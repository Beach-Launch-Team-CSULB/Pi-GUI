...
class StateButtons:
    ...
    def Logic(self):
        ...
        #If not in test mode
             ...
             #the button clicked is the next in the flow path
            if self.prevState.stateName == StateButtons.CurrState or (
                ...
                if self.prevState.stateName != "Test":
                    ...
                    self.passiveState.config(text="Passive", fg="red")  # , font=fontSize)
                self.StateActuaction()
...
class VentAbortFrame:
    ...
    def LogoPress(self, event):
        BrandonLabel = Label(self.parent, text="U WOT M8", bg="black", fg="white", font=("Verdana", 50))
...
class PropulsionFrame:
    ...
    valves = (
        ...
        ('LV', .45, .01, 18, 1, 36, 37),
        ('LDR', .105, .275, 19, 1, 38, 39),
        #('LDV', .265, .275, 20, 1, 40, 41),
        ('FV', .45, .85, 21, 1, 42, 43),
        ('FDR', .105, .55, 22, 1, 44, 45),
    ...
    sensors = [
        #["K bottle", 0.06, 0.0225, 0.075, 0.00, 84, yellow],
    ...
    def __init__(self, parent):
        ...
        self.propFrame.create_line(25, 100, 25, 337, fill=yellow, width=5)  #
