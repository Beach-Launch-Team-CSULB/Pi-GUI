...
class StateButtons:
    ...
    def VentAbortInstantiation(self):
        ...
        button = Button(self.parent, text=self.args[0], command=lambda: leftframe.StateReset(),fg='red', bg='grey', font=self.fontSize)
...
class VentAbortFrame:
    ...
    def __init__(self, parent):
        ...
        self.autoseqence = Label(ventAbortFrame, text="Boom Boom \n wont go boom boom", bg="black", fg="Green", font=fontSize)
        self.autoseqence.place(relx=.5, rely=0.2)
        ...
        if StateButtons.CurrState != "Override":
            ...
            for i in range(len(StatesFrame.States)):
                if StatesFrame.States[i][0] == StateButtons.CurrState:
                    self.reminderButtonOfCurrState = Button(leftframe.stateFrame, text= StateButtons.CurrState,fg='orange', bg='black', bd = 5)
                    # Goes to logic function when button is pressed
